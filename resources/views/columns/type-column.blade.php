@php
    ...
    if ($getType() && $getFor()) {
        $type = $model::where('key', $getState())->where('for', $getFor())->where('type', $getType())->first();
    } else {
        $type = $model::where('key', $getState())->first();
    }
...
@if ($value || config('filament-types.empty_state'))
    ...
    <span @if ($description && $getAllowDescription()) x-tooltip="{content: '{{ $description }}', theme: $store.theme }" @endif
        style="{{ implode([
